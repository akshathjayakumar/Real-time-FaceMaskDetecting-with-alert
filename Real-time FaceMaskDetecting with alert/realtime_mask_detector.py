...
# Define a function to detect and predict face masks
def detect_and_predict_mask(frame, face_net, mask_net):
	...
	for i in range(0, detections.shape[2]):
		...
		if confidence > 0.5:
			...
			detected_locs.append((startX, startY, endX, endY))

	if len(detected_faces) > 0:
		detected_faces = np.array(detected_faces, dtype="float32")
		detected_preds = mask_net.predict(detected_faces, batch_size=32)

	return (detected_locs, detected_preds)
...
while True:
	...
	for (box, pred) in zip(locs, preds):
		...
		if label == "No Mask":
			# Capture the image
			img_name = "mask_violation.png"
			cv2.imwrite(img_name, frame)

			# Send email
			msg = MIMEMultipart()
			msg.attach(MIMEImage(open(img_name, "rb").read()))
			msg['Subject'] = "Face Mask Violation"
			...
			server.sendmail("sender_email@gmail.com", "receiver_email@gmail.com", msg.as_string())
			server.quit()
